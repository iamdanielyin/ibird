 ...
 * @param join 条件连接方式
 */
function defaultFindCondition(schema, paths, keyword, join = '1') {
    ...
    return join == '1' ? {$or: condition} : {$and: condition};
}
...
app.create = function (object, req, res) {
    ...
    hooks["pre-create"](req.body, (body)=> {
...
app.update = function (object, req, res) {
    ...
    hooks["pre-update"](req.body, (body) => {
        Model.update(body.cond, body.doc, function (err, result) {
            if (err) {
...
app.list = function (object, req, res) {
    ...
    const conditions = keyword ? defaultFindCondition(schema, Model.schema.paths, keyword, join) : {};// _.omit(req.query, 'flag', 'page', 'size', 'sort');
    const query = Model.find(assign({}, conditions, req.authscope || {}));
