 ...
 * @returns {Array}
 */
function columnsPart(table, keys, schema, table_alias) {
    const columns = [], fields = schema.fields;
    table_alias = table_alias || table.name;
    _.keys(schema.fields).forEach(function (code) {
        const field = fields[code];
        if (field.ref) {
            const key = field.ref;
...
app.create = function (object, req, res) {
    ...
    hooks["pre-create"](table, (table) => {
        mssql.bulk(table, rowCount => {
            ...
            const result = {rowCount: rowCount};
            res.json(assign(result, (res.data || {})));
        }, err => {
...
app.update = function (object, req, res) {
    ...
    hooks["pre-update"](command, (command) => {
        mssql.transaction(command, function () {
            ...
            return res.json(assign(doc, (res.data || {})));
        }, function (err) {
...
app.list = function (object, req, res) {
    ...
    const join_prefix = (join == '2' ? ' AND ' : ' OR ');
    ...
    const columns = columnsPart(table, keys, schema, 't1');
    ...
    if (flag != 1) {
        ...
        ${_.isArray(conditions) && conditions.length > 0 ? 'AND (' + conditions.join(join_prefix) + ')' : ''}`;
    } else {
        //全部查询
        sort = (sort.length == 0) ? [`[t1].[${key}]`] : sort;
        command = `SELECT ${columns.join(',')} FROM ${table.name} AS t1 ${joins.join(' ')} ${_.isArray(conditions) && conditions.length > 0 ? 'WHERE (' + conditions.join(join_prefix) + ')' : ''} ORDER BY ${sort.join(',')}`;
    }

    mssql.query(command, function (result) {
        population(schema, result, function (result) {
            command = `SELECT COUNT(${key}) AS counts FROM ${table.name} as t1 ${_.isArray(conditions) && conditions.length > 0 ? 'WHERE (' + conditions.join(join_prefix) + ')' : ''}`;
            mssql.query(command, function (rs) {
...
app.one = function (object, req, res) {
    ...
    let command = `SELECT ${columns.join(',')} FROM ${table.name} AS t1 ${joins.join(' ')} WHERE ${conditions.join(' and ')}`;
    if (true == schema.singleton) {
        command = `SELECT TOP 1 ${columns.join(',')} FROM ${table.name} AS t1 ${joins.join(' ')}`;
    }
    mssql.query(command, function (rs) {
        population(schema, rs, function (rs) {
            rs = rs && rs.length > 0 ? rs[0] : rs;
            return res.json(assign(rs, (res.data || {})));
        });
    }, function (err) {
        log.error(i18n.value('log_read_object_error', [modelCode, JSON.stringify(err)]));
        return res.json({err: {message: i18n.value('res_read_object_error'), detail: err}});
...
app.export = function (object, req, res) {
    ...
    const joins = joinPart(table, schema);
    const where = all == '1' ? '1=1' : key + " IN ('" + _ids.join("','") + "')";
    let command = `SELECT ${columns.join(',')} FROM ${table.name} ${joins.join(' ')} ${'WHERE ' + where}`;
