    ...
    User.findOne({code: username}).populate('org').exec((err, data) => {
        ...
        Role.find({_id: {$in: data.roles || []}}, (err, roles) => {
            ...
            const authResult = (orgs) => {
                ...
                result.data.tempRoleCodes = tempRoleCodes;
    ...
    return {
        ...
        "schemas": [
            ...
            },
            {
                ...
                "hooks": {
                    "pre-create": function (data, callback) {
                        const logger = ibird.log(module);
                        const redis = ibird.redis;
                        ;
                        const key = 'hanltu_orderno_key';
                        redis.get(key, (err, result) => {
                            if (err || !result) return callback(data);
                            result = result || 0;
                            result = parseInt(result) + 1;
                            logger.info('订单号为：' + result);
                            redis.set(key, result);//更新redis
                            data.orderid = result;//设置orderid
                            callback(data);//回调保存
                        });
                    },
