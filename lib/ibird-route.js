...
function defaultFindCondition(paths, keyword) {
    ...
    Object.keys(paths).map(function (key) {
        if (paths[key].instance != 'String' || paths[key].options.inputType == 'password' || ['ts', 'dr', '__v', '_id'].indexOf(key) != -1)  return;
        const object = {};
...
function population(paths, query) {
    Object.keys(paths).map(function (key) {
        if (['ref', 'refs'].indexOf(paths[key].options.inputType) == -1)  return;
...
function registerRoutes(app, key, Model, routesAuthDeclrString) {
    ...
    const methods = routesAuthDeclrString ? routesAuthDeclrString.toUpperCase().split(',') : [];
    //默认鉴权
