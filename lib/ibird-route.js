...
function defaultFindCondition(paths, keyword) {
    ...
    Object.keys(paths).map(function (key) {
        if (paths[key].instance != 'String')  return;
        const object = {};
...
function registerRoutes(app, key, Model, routesAuthDeclrString) {
    ...
    app.get('/' + moduleCode + '/' + modelCode, function (req, res) {
        const conditions = _.omit(req.query, 'flag', 'page', 'size', 'sort');
        const keyword = req.query.keyword || null;
        const query = Model.find(keyword ? defaultFindCondition(Model.schema.paths, keyword) : {});
        ...
        query.sort(sort).exec(function (err, result) {
            ...
            Model.count(conditions, function (err, count) {
                ...
                if (flag != 1) {
                    ...
                    let start = page > 1 ? ((page - 1) * size) : 1;
                    data.start = start < data.totalelements ? start : 1;
                    let end = data.start + size;
                    data.end = end > data.totalelements ? data.totalelements : end;
                }
