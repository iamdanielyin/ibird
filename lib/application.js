...
class App extends Koa {
    ...
    import(addon, options) {
        ...
        options = Object.assign({ autoMountRoutes: true, autoUseMiddleware: true, enableAlias: {} }, options);
        ...
        if (addon.enable) {
            ...
            for (let key in enable) {
                ...
                // 处理别名
                if (options.enableAlias && options.enableAlias[key]) {
                    key = options.enableAlias[key];
                }
    ...
    play(port, callback) {
        ...
        if (port) {
            ...
            callback = callback || (() => console.log(`Running on ${localURL}`));
            this.listen(port, callback);

            // Event - ibird:app:listen
            this.emit('ibird:app:listen', this, localURL);
        }
