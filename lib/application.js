...
class App extends Koa {
    ...
    config(opts) {
        ...
        if (opts) {
            ...
            // 合并配置
            this._config = utility.assign((this._config || {}), opts);
    ...
    import(addon, options) {
        ...
        options = utility.assign({ autoMountRoutes: true, autoUseMiddleware: true, apiAlias: {} }, options);
 ...
 * 挂载默认日志API
 */
function defaultLocaleAPIs(opts) {
    if (typeof this.locales !== 'object') {
        this.locales = {};
    }
    if (typeof this.getLocaleString === 'function') return;
    const defaultLocale = opts.defaultLocale || 'en_US';

    this.L = this.locale = this.getLocaleString = (key, params, localeOrName) => {
...
function initialize(opts) {
    ...
    if (opts.uploadDir) {
        ...
        opts.bodyOpts.formidable = utility.assign({
            keepExtensions: true,
    ...
    // Event - ibird:app:statics:pre
    this.emit('ibird:app:statics:pre', this, opts.statics);
    if (opts.statics) {
    ...
    // Event - ibird:app:statics:post
    this.emit('ibird:app:statics:post', this, opts.statics);
    ...
    // 挂载默认API
    defaultLogsAPIs.call(this, opts);
    defaultLocaleAPIs.call(this, opts);
