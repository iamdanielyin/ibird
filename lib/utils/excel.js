...
app.export = (file, sheets) => {
    ...
    sheets.forEach((item) => {
        ...
        item.displayRowIndex = !_.isNaN(_.toNumber(item.displayRowIndex)) ? _.toNumber(item.displayRowIndex) : 0;
        item.displayRowIndex = (item.displayRowIndex >= 0 && item.displayRowIndex < item.header.length) ? item.displayRowIndex : 0;
        let rows = _.isArray(item.header[item.displayRowIndex]) ? [item.header[item.displayRowIndex]] : [];
        //确保header内的值为数组类型
