...
app.export = (file, sheets) => {
    ...
    const array = [];
    sheets.forEach((item) => {
        if (!item || !item.name || !_.isArray(item.data) || item.data.length == 0) return;
        //{"name":"Sheet1","header":[["编码","名称","年龄"],["code","name","age"]],"json":true,"keyRowIndex":1,"data":[{"code":"1-1","name":"1-2","age":"1-3"}]}
        let rows = item.header = _.isArray(item.header) ? item.header : [];
        //确保header内的值为数组类型
        ...
        if (item.json == true) {
            //处理json数据
            if (item.header.length == 0) return;//为json类型时，header不允许为空，因为要指定JSON的key行
            //获取key的下标值
            const keyRowIndex = item.keyRowIndex && ((parseInt(item.keyRowIndex) || 0) < item.header.length) ? item.keyRowIndex : 0;
