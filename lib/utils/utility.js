...
function joinObject(dest, src) {
    _.keys(src).forEach(function (key) {
        if (!_.has(dest, key) || (typeof dest[key] !== typeof src[key]) || !(_.isObject(dest[key]))) return dest[key] = src[key];
 ...
 * @returns {*}
 */
app.tsExpired = function (ts, value, flag) {
    if (!ts || value == undefined) return false;
    const now = moment();
    const target = moment(ts, 'x').add(value, flag || ('s'));
    //过期时间在当前时间之前表示已过期
