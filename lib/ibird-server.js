 ...
 */
function registerModuleRoutes(moduleCode, moduleRoutes) {
 ...
 * @param moduleModels
 */
function modelRoute(moduleModels) {
...
exports.init = function init(adminConfig) {
    ...
    Object.keys(modules).map(function (code) {
        ...
        //注册模块声明路由
        registerModuleRoutes(code, module.routes);
        //注册模型和模型默认路由
        modelRoute(model(code, module.schemas));
    });
    exports.configs = configs;

    app.use(adminConfig.publicFullPath.substring(adminConfig.publicFullPath.lastIndexOf('/') + 1), express.static(adminConfig.publicFullPath));
