...
function defaultRoutes() {
    ...
    app.get('/config/menu', authMiddleware, function (req, res) {
        const menu = [];
        configs.modules.map(function (module) {
            if (!module) return;
            menu.push(utility.pick(module, ['code', 'label', 'icon', 'menu']))
        });
        return res.json(menu);
    });
    app.get('/config/schema', authMiddleware, function (req, res) {
        const schema = [];
        configs.modules.map(function (module) {
            if (!module) return;
            schema.push(utility.pick(module, ['code', 'label', 'icon', 'schemas']))
        });
