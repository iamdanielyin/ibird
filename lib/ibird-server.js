...
exports.initialize = function (adminConfig) {
    ...
    //客户端入口配置
    app.use(config.route, function (req, res) {
        return res.sendFile(path.resolve(__dirname, '../node_modules/ibird-client/' + (process.env.NODE_ENV == 'production' ? 'dist' : 'build') + '/index.html'));
    });
