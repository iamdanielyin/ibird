...
exports.initialize = function (configs) {
    ...
    model.init(ds, modules, exports.models, function () {
...
function defaultMenus(modules) {
    ...
    modules.forEach(function (mdl, i) {
        ...
        (mdl.schemas || []).forEach((schema, j) => {
            ...
            if (i == 0 && j == 0) object.items.push({
                "code": "menu",
                "label": "系统菜单",
                "icon": mdl.icon || 'circle-o',
                "uri": "com:menu"
            });
            object.items.push({
                code: schema.code,
 ...
 * @param configs
 */
function defaultTasksMenu(configs) {
    if (!configs || !configs.menu) return;
    const menu = configs.menu;
    const Param = exports.models['preset-param'].value;
    Param.findOne({code: 'MENU'}, (err, result) => {
        if (err) return logger.error(`维护系统菜单异常：${err.message}`);
        const param = result ? result : new Param({
            code: 'MENU',
            name: '系统菜单',
            org: '1',
            creater: 'ibird',
        ...
        param.value = menu;
        param.save((err, result)=> {
            if (err) return logger.error(`维护系统菜单异常：${err.message}`);
            logger.info(`维护系统菜单成功`);
        });
    });
}
        ...
        switch (flag) {
            ...
            case 'private':
                exports.token.authentication(access_token, function (err, r) {
                    ...
                    //查询菜单参数
                    const Param = exports.models['preset-param'].value;
                    Param.findOne({code: 'MENU'}, (err, param) => {
                        if (err) return logger.error(`查询系统菜单异常：${err.message}`);
                        if (param && param.value) result.menu = param.value;
                        //返回前端
                        const json = i18n.format(utility.deepClone(result));
                        return res.json(json);
                    });
                ...
                switch (type) {
                    ...
                    case 'mssql':
                        ...
                        mssql.query(command, function (rs) {
                            ...
                            logger.error(`维护系统资源异常：${err.message}`);
                            --size;
                            callback();
                        });
                        break;
