...
app.delete = function (object, req, res) {
    ...
    hooks["pre-delete"](req.body, (body) => {
        Model.update(body, {dr: '1'},{multi:true}, function (err, result) {
            if (err) {
...
app.list = function (object, req, res) {
    ...
    hooks["pre-list"](req, conditions, (conditions) => {
        if (_.isArray(conditions.$and)) {
            conditions.$and.push({dr: {$ne: '1'}});
        } else {
            Object.assign(conditions, {dr: {$ne: '1'}});
        }
        const query = Model.find(conditions);
        const flag = parseInt(req.query.flag) || 0;
        const page = parseInt(req.query.page) || 1;
        const size = parseInt(req.query.size) || 20;
        const sort = req.query.sort || (schema.query && schema.query.sort ? schema.query.sort : "-modified -created");//按修改时间逆序
        ...
        query.sort(sort).exec(function (err, result) {
            if (err) {
                log.error(i18n.value('log_read_object_error', [modelCode, err.message]));
                return res.json({err: {message: i18n.value('res_read_object_error'), detail: err.message}});
            }
    ...
    query.exec(function (err, result) {
        ...
        _.keys(schema.fields).forEach(key => {
            ...
            if (!field.ref) {
                ...
                if (!field.refOptions) return;
                const refschema = (models[field.ref] || {}).schema;
                if (!refschema) return;
                const reffields = refschema.fields;
                for (const refkey of _.values(field.refOptions)) {
                    const reffield = reffields[refkey];
                    if (!reffield || !reffield.label) continue;
                    keys.push(`${key}_${refkey}`);
                    displays.push(`${field.label}-${reffield.label}`);
                }
            }
        });
        result.forEach(item => {
            ...
            //处理枚举、引用类型
            _.keys(item._doc).forEach(key => {
                const field = schema.fields[key];
                if (!key || !field || !field.ctrltype) return;
                switch (field.ctrltype) {
                    case 'boolean-checkbox':
                        if (!(_.isObject(field.items) && !_.isArray(field.items))) return;
                        const array = _.isArray(item[key]) ? item[key] : (item[key] ? item[key].split(',') : []);
                        const values = [];
                        if (array.length <= 0) return;
                        array.forEach(_val => values.push(field.items[_val]));
                        item[key] = values.join(',');
                        break;
                    case 'boolean-radios':
                        if (!(_.isObject(field.items) && !_.isArray(field.items))) return;
                        item[key] = field.items[item[key]];
                        break;
                    case 'ref':
                        const refobj = item[key];
                        if (!refobj) return;
                        for (const refkey of _.values(field.refOptions)) {
                            if (!refkey || !refobj[refkey]) continue;
                            const refschema = (models[field.ref] || {}).schema;
                            if (!refschema) continue;
                            const reffield = (refschema.fields || {})[refkey];
                            item[`${key}_${refkey}`] = reffield.items ? reffield.items[refobj[refkey]] : refobj[refkey];
                        }
                        break;
                    case 'refs':
                        break;
