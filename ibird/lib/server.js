...
exports.transmdl = require('./middleware/transmdl');
...
function defaultTasksUserOrg(config) {
    ...
    const addorg = () => {
        Org.create(assign({}, parent, {
            ...
            if (err) return logger.error(`创建默认默认机构成功异常：${err.message}`);
            logger.info(`创建默认默认机构成功：${result.code}`);
        });
    };
    Org.findOne({code: '1'}, (err, result) => {
        if (err) return logger.error(`查询默认机构异常：${err.message}`);
...
function defaultTasksResources(configs) {
    ...
    modules.forEach(function (mdl) {
        ...
        (mdl.schemas || []).forEach(function (schema) {
            ...
            parent.tag = `${mdl.label + ' ' + schema.label}（操作权限）`;
            //接口权限
    ...
    Role.findOne({$or: [{_id: 'SUPER'}, {code: 'SUPER'}]}, (err, result) => {
        ...
        Resource.find({}, '_id', (err, exists) => {
            exists = _.keys(_.groupBy(exists,'_id'));
            if (result && result._id) {
                ...
                if (result.code || result.name || _.isArray(result.resources)) {
                    ...
                    superole.resources = _.isArray(exists) ? superole.resources.concat(exists) : superole.resources;
...
function defaultRoutes(ruprefix = '') {
    ...
    app.get(ruprefix + '/home', exports.transmdl, function (req, res) {
        ...
        exports.configs.modules.forEach(mdl => {
            if (!mdl || !_.isArray(mdl.schemas)) return;
            mdl.schemas.forEach(schema => {
                const authid = `${mdl.code}_${schema.code}_list_ui_view`;
                const key = `${mdl.code}-${schema.code}`;
                if (roles.indexOf(authid) == -1) return;
