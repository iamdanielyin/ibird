...
const AdminForm = React.createClass({
    ...
    getInitialState(){
        ...
        return {
            ...
            tempRoleCodes: token.data.tempRoleCodes || [],
            org: token.data.org,
            view: schema.singleton == true ? 1 : query.view
        };
    },
    getTempRoleCodesClass(){
        const tempRoleCodes = _.isArray(this.state.tempRoleCodes) ? this.state.tempRoleCodes : [];
        tempRoleCodes.forEach((item, i) => {
            tempRoleCodes[i] = `hanltu-temp-roles-` + item;
        });
        return tempRoleCodes.join(' ');
    },
    assignContext(){
    ...
    fetchData(){
        ...
        fetch(RouteUtils.CUSTOM('/' + this.state.moduleCode + '/' + this.state.modelCode + '/' + (schema.singleton == true ? uuid.v1() : self.state.i)) + '?' + query).then(function (res) {
            return res.json();
        }).then(function (json) {
            if (json.err) return toastr.error(json.err.message, null, ToastrUtils.defaultOptions);
            if (!json || _.keys(json).length == 0) return;
            //TODO 临时增加汉兰图订单状态
            if (json.status) $('.hanltu-temp-order-class').addClass('hanltu-temp-order-status-' + json.status);
            if (!json || _.keys(json).length == 0) return;
            const state = {data: json};
            state.i = json[schema._id || '_id'] || self.state.i;
            self.setState(state, () => self.refreshCtrls());
            toastr.success('加载数据成功', null, assign({},ToastrUtils.defaultOptions, {progressBar: true, preventDuplicates: true, timeOut: 1000}));
        });
    },
    getIdentifier(key){
        if (!key) return null;
    ...
    formGeneration(){
        ...
        if (this.state.roles.indexOf(viewAuth) == -1) {
            toastr.error('无界面访问权限，自动跳转到主页', null, assign({},ToastrUtils.defaultOptions, {progressBar: true, preventDuplicates: true, timeOut: 2000}));
    ...
    refreshiCheckOptions(type, identifier, ajax, item){
        ...
        if (!group) {
            ...
            _.keys(groups).forEach(title => {
                ...
                $array.push(`<fieldset><legend>${title}</legend>${fieldset.join('')}</fieldset>`);
            });
        }
        ...
        return (
            <div
                ...
                <div className={`col-xs-12`}>
                    <div className="hanltu-temp-order-class">
                        <div className={`hanltu-temp-roles-class ${this.getTempRoleCodesClass()}`}>
                            <div className="box box-primary">
                                ...
                                <form role="form" className={config.inline ? "form-inline" : ""}>
